...
def car_watch(carname):
    ...
    for header, prefix_dict in header_groups.items():
        ...
        for prefix, files in prefix_dict.items():
            ...
            for file in files:
                df = pd.read_csv(file)
                dfs.append(df.iloc[:, car_dict[carname]])

            combined = pd.concat(dfs, axis=1)

            # ★ 修正： 100 から引く（反転）
            percentage = combined.mean(axis=1) * 100

            time = percentage.index * 0.1

            plt.plot(time, percentage, label=prefix, linewidth=2)

        plt.title(carname + " % per Group (Inverted)\nHeader: " + header)
        plt.xlabel("Time (s)")
        plt.ylabel("PrimaryButtonFlag = 0 (%)")
        plt.ylim(0, 100)
        plt.grid(True)
        plt.legend()

        safe_header = header.replace(",", "_")
        save_name = os.path.join(output_dir, f"AverageGraph_watch{carname}_{safe_header}.png")
